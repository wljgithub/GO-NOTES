...
// 遍历一个数组里的元素，range返回元素的索引和元素的值，在这里我不想用元素的索引，所以用匿名变量来代替
func main() {
	arr := []int{1, 2, 3, 4, 5}
	// _ 下划线代表声明一个匿名变量
	for _, v := range arr {
...
常量和变量不同的地方在于，常量是在编译阶段就确定好的值，初始化后无法进行更改

声明一个常量也很简单，用const关键字：
...
- 常量不能用:= 方式进行声明，
- 因为常量是编译阶段值就已经确定好的，所以不能将运行阶段才能获取得到的值赋给常量，比如 const a = time.Now()
